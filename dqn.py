...
class Net(nn.Module):
    ...
    def forward(self, x):
        ## TODO: Implement the forward of the value network
        return self.main(x)
    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            ...
            if total_steps < args.warmup:
                # In the beginning, use the random policy to sample episode
            ...
            if total_steps >= args.warmup:
                # Before warmup, agent try to update itself in each step
...
def main():
    ...
    ## main ##
    env = gym.make('LunarLander-v2')  # Build environment
    agent = DQN(args)  # Init DQN agent
    writer = SummaryWriter(args.logdir)  # TODO: Need survey
